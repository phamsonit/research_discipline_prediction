...
# References:
#   i.      Labeled LDA: A supervised topic llda-model for credit attribution in multi-labeled corpora, Daniel Ramage...
...
class LldaModel:
    ...
    @field theta_star: percentage of share of the sum of the labelled

    """
    def __init__(self, alpha_vector="50_div_K", eta_vector=None, labeled_documents=None, common_topic=True, theta_star=None):
        """
    ...
    def _load_labeled_documents(self, labeled_documents):
        ...
        for document, labels in labeled_documents:
            ...
            # doc_words = document
            # print(document)
            doc_words = LldaModel._tokenize_document(document)
            doc_corpus.append(doc_words)
            if labels is None:
                labels = []
            if self.common_topic:
                labels.append("common_topic")  # modified by me

            avg_num_labels += len(labels)

            labels_corpus.append(labels)
            all_words.extend(doc_words)
            all_labels.extend(labels)
        avg_num_labels = avg_num_labels / len(labeled_documents)
        ...
        # we appended "common_topic" to each doc at the beginning
        # so we need minus the number of "common_topic"
        # LN is the number of original labels
        if self.common_topic:
            self.LN = len(all_labels) - self.M
        else:
            self.LN = len(all_labels)

        if self.theta_star is not None:
            # lamda is set as 0.1 be default not 0 as in the original algorithm
            # by setting lamda = 0.1, each document samples from all topics
            lda = (avg_num_labels / (self.K + 1 - avg_num_labels)) * (1 / self.theta_star - 1)
            # print(avg_num_labels, " ", self.theta_star, " ", lamda)
        else:
            lda = 0

        self.Lambda = np.zeros((self.M, self.K), dtype=float) + lda
        for m in range(self.M):
        ...
        for m in range(self.M):
            ...
            # print("numerator_vector: ", numerator_vector)
            if self.theta_star is not None:
                # calculate Direchlet distribution of alpha vector
                theta_vector = np.random.dirichlet(numerator_vector)
                # print("dirichlet vector: ", theta_vector)
            else:
                theta_vector = 1.0 * numerator_vector / sum(numerator_vector)
            ...
            z_vector = [LldaModel._multinomial_sample(theta_vector) for _ in range(len(self.W[m]))]
    ...
    def _gibbs_sample_training(self):
        ...
        for m in range(self.M):
            ...
            for t, z, n in zip(self.W[m], self.Z[m], range(len(self.W[m]))):
                ...
                # print("theta_vector: ", numerator_theta_vector)
    ...
    def _gibbs_sample_inference(self, term_vector, iteration=30, times=10):
        """
    ...
    def training(self, iteration=30, log=False):
        """
        training this llda-model with gibbs sampling
    ...
    def beta(self):
        ...
        This name "beta" comes from
            "Labeled LDA: A supervised topic llda-model for credit attribution in multi-labeled corpora, Daniel Ramage..."
        topic-term distribution
    ...
    def log_perplexity(self, documents=None, iteration=30, times=10):
        """
        log perplexity of LDA topic llda-model, use the training data_alpha_eta_testing if documents is None
        ...
        if not documents:
            ...
            test_corpus = []  # [document.split() for document in documents] : modified by me
            for document in documents:
                tokens = LldaModel._tokenize_document(document)
    ...
    def perplexity(self, documents=None, iteration=30, times=10):
        """
        perplexity of LDA topic llda-model, we use the training data_alpha_eta_testing if documents is None
        Reference:  Parameter estimation for text analysis, Gregor Heinrich.
        :param: documents: test set
    ...
    def __repr__(self):
        return "\nLabeled-LDA Model:\n" \
               ...
               "\t" % (self.K, self.M, self.T, self.WN, self.LN, self.alpha_vector[0], self.eta_vector[0],
                       self.theta_star, self.perplexity())
        pass
    ...
    def save_model_to_dir(self, dir_name, save_derivative_properties=False):
        """
        save llda-model to directory dir_name
        :param save_derivative_properties: save derivative properties if True
            ...
            to save derivative properties to disk:
                it will reduce the time of loading llda-model from disk (read properties directly but do not compute them)
    ...
    def load_model_from_dir(self, dir_name, load_derivative_properties=True):
        """
        load llda-model from directory dir_name
    ...
    def update(self, labeled_documents=None):
        """
        update llda-model with labeled documents, incremental update
        :return: None
        """
        self.all_perplexities = []
    ...
    def is_convergent(self, method="PPL", delta=0.001):
        ...
        :param method: the method of determining whether the Markov chain converges
                "PPL": use the perplexities of training data_alpha_eta_testing
        ...
        :return: True if llda-model is convergent
        """
        if method == "PPL":
